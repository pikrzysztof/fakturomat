...
\begin{document}
 ...
 \textit{Nabywca:} \\
  ...
  \\ \ \\
  \small
  \begin{tabular}{p{0.5cm} p{6.3cm} p{1cm} p{0.7cm} p{0.7cm} p{1.5cm} p{1cm} p{0.8cm} p{1cm} p{1.7cm}}
  \hline 
  Lp & Nazwa usługi & PKWIU & j.m. & Ilość & Cena jedn.\newline netto & Wartość netto & VAT & Podatek & Wartość z podatkiem \\ 
   \hline
   {% for poz in pozycje %}
   {{ loop.index }} & {{ poz.wyswietlana_nazwa or poz.nazwa }} & {{ poz.pkwiu }} & {{ poz.jm }} & {{ poz.ile }} & {{ poz.netto_za_sztuke }} & {{ poz.wartosc_netto }} & {{ poz.podatek_proc }} \% & {{ poz.podatek_zl }} & {{ poz.brutto }} \\
   {% endfor %}
  \hline
  & & & & & Razem: & \netto & & \vat & \brutto \\
%   & & & & W tym: & \netto &  & \vat & \brutto \\
