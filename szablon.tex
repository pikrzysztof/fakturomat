...
\begin{document}
 ...
 \ \\
   ...
   {% if faktura.klient.adres_druga_linijka %}
   {{ faktura.klient.adres_druga_linijka }} \\
   {% if faktura.klient.adres_trzecia_linijka %}
   {{ faktura.klient.adres_trzecia_linijka }} \\
   {% if faktura.klient.adres_czwarta_linijka %}
   {{ faktura.klient.adres_czwarta_linijka }} \\
   {% endif %}
   {% endif %}
   {% endif %}
 ...
 \begin{tabular}{r l l r p{2cm} p{2cm} l r p{2cm}}
  ...
  Lp & Nazwa usługi & j.m. & Ilość & Cena jedn.\newline netto & Wartość netto & VAT & Podatek & Wartość z podatkiem \\ 
   \hline
   {% for poz in pozycje %}
   1 & {{ poz.wyswietlana_nazwa or poz.nazwa }} & {{ poz.jm }} & {{ poz.ile }} & {{ poz.netto_za_sztuke }} & {{ poz.wartosc_netto }} & {{ poz.podatek_proc }} \% & {{ poz.podatek_zl }} & {{ poz.brutto }} \\
   {% endfor %}
  \hline
  & & & & Razem: & \netto & & \vat & \brutto \\
%   & & & & W tym: & \netto &  & \vat & \brutto \\
